...
import {IDexLauncher} from "./interfaces/IDexLauncher.sol";
...
library TransferHelper {
    function safeTransferFrom(address token,address sender, address to, uint256 value) internal {
        (bool success, bytes memory data) =
            token.call(abi.encodeWithSelector(IERC20Minimal.transferFrom.selector,sender, to, value));
...
contract DexLauncher is IDexLauncher, Auth, ReentrancyGuard {
    ...
    constructor(
        ...
        IERC20(wgas_).approve(uniswapV3Factory_, type(uint256).max);
    ...
    function _createPool(address tk) internal returns (address pool_) {
        ...
        uint160 sqrtPriceX96 = _getSqrtRatioAtTick(_poolTick);
        ...
        returns (uint256 tokenId, uint128 liquidity, address token0, uint256 amount0, address token1, uint256 amount1)
    {
...

        {
            ...
            IWETH9(WGAS).deposit{value: gasAmountToMint}();
...

        {
            ...
            //emit PoolLiquidityMinted(tk, tokenId, amount0, amount1, liquidity);
        }
    ...
    function uniswapV3SwapCallback(int256 amount0Delta, int256 amount1Delta, bytes memory) external {
        ...
        // }
        IWETH9(WGAS).transfer(msg.sender, amount0Delta > amount1Delta ? uint256(amount0Delta) : uint256(amount1Delta));
    }
